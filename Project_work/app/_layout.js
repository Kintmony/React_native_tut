...
import { useFonts } from "expo-font"; //for enabling custom fonts(hook)
...
const Layout = () => {
  ...
  const onLayoutRootVView = useCallback(async () => {
    if (fontsLoaded) {
      await SplashScreen.hideAsync(); //check if fonts have been loaded before showing the Home screen
    }
  }, [fontsLoaded]); //like a useEffect
  if (!fontsLoaded) return null; //if fonts are not loaded
  return <Stack onLayout={onLayoutRootVView} />; //if fonts are loaded return the view
};
